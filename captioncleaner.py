...
def process_single_chunk(chunk_objects, system_prompt, provider, info_str):
    ...
    response_text = provider.query(prompt, system_prompt)
...
def clean_pass(subs, system_prompt, provider, chunk_size, offset=0, pass_name="Pass 1"):
    processed_subs = []
    n = len(subs)
    ...
    if offset > 0:
        ...
        processed_subs.extend(process_single_chunk(chunk_objects, system_prompt, provider, f"[{pass_name}] Processing blocks 1 to {end_idx}"))
        current_idx = end_idx
    ...
    for i in range(current_idx, n, chunk_size):
        ...
        processed_subs.extend(process_single_chunk(chunk_objects, system_prompt, provider, info_str))

    return processed_subs

def process_srt(input_path, output_path, system_prompt, provider, chunk_size):
    ...
    subs = clean_pass(subs, system_prompt, provider, chunk_size, offset=0, pass_name="Pass 1")

    # Pass 2: Offset chunking (Review)
    print(f"\n--- Starting Pass 2 (Review) ---")
    # Offset by half the chunk size to bridge boundaries
    offset = chunk_size // 2
    subs = clean_pass(subs, system_prompt, provider, chunk_size, offset=offset, pass_name="Pass 2")
